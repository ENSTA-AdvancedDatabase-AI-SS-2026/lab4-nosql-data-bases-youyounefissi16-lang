...
def get_product_rank(r, product_id) -> Optional[int]:
    ...
    if rank is None:
        return None
    return rank + 1  # Convertir de 0-based à 1-based
...
def get_products_between_ranks(r, start_rank: int, end_rank: int) -> list:
    ...
    """
    # Convertir rangs 1-based en indices 0-based
...
def get_category_leaderboard(r, category: str, product_ids: list, n: int = 5) -> list:
    """
    Bonus: Classement par catégorie.
    Utilise un sous-ensemble des produits d'une catégorie.
    """
    # Créer un Sorted Set temporaire pour la catégorie
    tmp_key = f"leaderboard:category:{category}"
    pipe = r.pipeline()
    for pid in product_ids:
        score = r.zscore(LEADERBOARD_KEY, str(pid))
        if score is not None:
            pipe.zadd(tmp_key, {str(pid): score})
    pipe.expire(tmp_key, 3600)  # TTL 1h
    pipe.execute()

    results = r.zrevrange(tmp_key, 0, n - 1, withscores=True)
    return [{"product_id": pid, "sales": score} for pid, score in results]
...
if __name__ == "__main__":
    ...
    print(f"Produits classés 2 à 4: {get_products_between_ranks(r, 2, 4)}")
